\begin{figure}[t]
\centering
\begin{tikzpicture}[node distance=2.5cm,>=stealth',auto,every state/.style={thick}every node/.style={inner sep=-.4cm,minimum size=-1cm,scale=0.8},scale=0.8]] 
	\node (init) {};
	\node[state] (p_ext) [right=.7cm of init] {};
	\node[state] (p) [right of=p_ext] {};
  \node[state] (p1) [above right of=p] {};
	\node[state] (q1) [below right of=p] {};
	\node[state] (f) [right=5cm of p] {};
